/*
FX使用說明:
宣告一個extern FX *fx (這是全域變數)
要加FX的時候 在想要加入的地方加上 
fx->NewFX("特效名字", float[3](絕對位置或相對位置), "一個特別的名字 刪除會用到", 人物(若使用在人身上 可加可不加))
在想要結束的地方加上
fx->Delete("你命名的名字");
我有實作一些函式可以直接用 可以看125行左右 加入攻擊的例子
以及554行 如何刪除攻擊特效
*/
#include "Lyubu.h"
#include "utils.h"
		...
		switch(code)
		{
			...
			case FY_L:
				if(value)
				{
					...
					fx->NewFX("Thunder", pos, "lyubu_attack", *this);
					state.add(LYUBU_ATT);
					curAttID = nAtt3ID;
				}
				break;
